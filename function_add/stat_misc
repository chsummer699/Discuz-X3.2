function getstatvars_forumstat($fid) {
 ...
 // $statvars['uid'] = $uid;
  ...
  foreach(C::t('forum_forum')->get_authorid($fid,"forum_post") as $data){  
	...
	$statvars['num'] = $num;
$count = 1;	
	foreach(C::t('forum_forum')->get_post_by_fid($fid,$start_line,$end_line,"forum_post") as $data){
		if($data['first']){ 
			...
			if($data['rate']){
				...
				$bug[$count]['message'] = $data['subject'].":<br> ".$data['message'];
